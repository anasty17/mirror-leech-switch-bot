#!/usr/bin/env python3
from swibots import CommandHandler, CallbackQueryHandler, regexp, filters, MessageHandler
...
from bot import config_dict, user_data, DATABASE_URL, MAX_SPLIT_SIZE, DRIVES_IDS, DRIVES_NAMES, INDEX_URLS, aria2, GLOBAL_EXTENSION_FILTER, status_reply_dict_lock, Interval, aria2_options, aria2c_global, download_dict, qbit_options, get_client, LOGGER, bot
from bot.helper.switch_helper.message_utils import sendMessage, sendFile, editMessage, update_all_messages, deleteMessage
...
Timeout: 60 sec'''
        ...
        buttons.ibutton('Close', "botset close")
        if key in ['SUDO_USERS', 'OWNER_ID', 'AUTHORIZED_CHATS', 'DATABASE_URL', 'BOT_TOKEN', 'DOWNLOAD_DIR']:
...
bot.add_handler(CommandHandler(BotCommands.BotSetCommand,
                bot_settings, filter=CustomFilters.sudo))
bot.add_handler(CallbackQueryHandler(edit_bot_settings,
                filter=regexp("^botset") & CustomFilters.sudo))
