...
from bot.helper.switch_helper.message_utils import sendMessage, sendStatusMessage, deleteMessage
...
async def get_confirm(ctx):
    ...
    if hasattr(dl, 'listener'):
        ...
        await ctx.event.answer("Not in download state anymore! Keep this message to resume the seed if seed enabled!", show_alert=True)
        return
...
bot.add_handler(CommandHandler(BotCommands.BtSelectCommand,
                select, filter=CustomFilters.authorized))
