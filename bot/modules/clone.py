...
from bot.helper.switch_helper.message_utils import sendMessage, deleteMessage, sendStatusMessage
...
async def rcloneNode(client, listener, link, dst_path, rcf):
    if link == 'rcl':
...
async def gdcloneNode(client, listener, link, dest_id):
    ...
    if is_gdrive_link(link) or is_gdrive_id(link):
        ...
        if sa:
            drive.use_sa = True
        if files <= 10:
            msg = await sendMessage(listener.message, f"Cloning: <copy>{link}</copy>")
        else:
            msg = ''
            gid = ''.join(SystemRandom().choices(ascii_letters + digits, k=12))
            async with download_dict_lock:
...
async def _cloneNode(client, message):
    input_list = message.message.split(' ')
    ...
    async def __run_multi():
        if multi > 1:
            ...
            await sleep(5)
            await _cloneNode(client, nextmsg)
    ...
    if is_rclone_path(link):
        if not await aiopath.exists('rclone.conf') and not await aiopath.exists(f'rclone/{listener.user_id}.conf'):
        ...
        if not config_dict['RCLONE_PATH'] and not listener.user_dict.get('rclone_path') and not dst_path:
            ...
            return
        await rcloneNode(client, listener, link, dst_path, rcf)
    else:
        ...
        if not config_dict['GDRIVE_ID'] and not listener.user_dict.get('gdrive_id') and not dst_path:
            ...
            return
        await gdcloneNode(client, listener, link, dst_path)
