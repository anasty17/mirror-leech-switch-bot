...
from bot.helper.ext_utils.bot_utils import cmd_exec, get_readable_file_size, new_task, get_readable_time, update_user_ldata
...
class RcloneList:
    ...
    async def __event_handler(self):
        ...
        handler = CallbackQueryHandler(
            pfunc, filter=regexp('^rcq') & user(self.user_id))
    ...
    async def get_path_buttons(self):
        ...
        msg += f'\nCurrent Path: <copy>{self.remote}{self.path}</copy>'
