...
from bot.helper.ext_utils.bot_utils import get_readable_file_size, get_readable_time, update_user_ldata
...
class gdriveList(GoogleDriveHelper):
    ...
    async def __event_handler(self):
        ...
        handler = CallbackQueryHandler(
            pfunc, filter=regexp('^gdq') & user(self.user_id))
    ...
    async def get_items_buttons(self):
        ...
        msg += f'\n\nCurrent ID: <copy>{self.id}</copy>'
        msg += f"\nCurrent Path: <copy>{('/').join(i['name'] for i in self.parents)}</copy>"
