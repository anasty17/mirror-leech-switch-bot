...
async def extract_bulk_links(message, bulk_start, bulk_end):
    ...
    if (reply_to := message.replied_to) and (file_ := reply_to.document) and (file_.mime_type == 'text/plain'):
        return await get_links_from_file(message.replied_to, bulk_start, bulk_end)
    elif text := message.replied_to.text:
